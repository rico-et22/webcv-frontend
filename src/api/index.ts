...
export interface GithubExchangeDto {
  /**
   * OAuth authorization code received from GitHub redirect
...
> extends HttpClient<SecurityDataType> {
  ...
  github = {
    /**
     * @description Call this after the user is redirected back from GitHub OAuth with a `code` param. Returns a short-lived GitHub access token and the authenticated GitHub username. The token is returned to the frontend and should be kept in memory — it is never stored server-side.
     *
     * @tags github
     * @name GithubControllerExchange
     * @summary Exchange GitHub OAuth authorization code for an access token
     * @request POST:/github/exchange
     * @secure
     */
    githubControllerExchange: (
      data: GithubExchangeDto,
      params: RequestParams = {}
    ) =>
    ...
    /**
     * @description Generates the static portfolio and pushes it to `<username>.github.io`. Creates the repo if it does not exist; force-pushes on subsequent deploys. Enables GitHub Pages automatically. Repo name is derived from the GitHub username — no configuration needed.
     *
     * @tags github
     * @name GithubControllerDeploy
     * @summary Deploy portfolio to GitHub Pages
     * @request POST:/github/deploy/{siteId}
     * @secure
     */
    githubControllerDeploy: (
      siteId: string,
      data: GithubDeployDto,
      params: RequestParams = {}
    ) =>
      this.request<any, void>({
        path: `/github/deploy/${siteId}`,
        method: "POST",
        body: data,
        secure: true,
        type: ContentType.Json,
